...
def advance(q, qdot, control_input, dt):
    ...
    # Initialize arrays for updated configuration angles and angular velocities
    θ_prime = np.zeros(N, dtype=float)
    ω_prime = np.zeros(N, dtype=float)
    
    # Update configuration and angular velocities
    for i in range(N):
        # Update angular velocity
        ω_prime[i] = qdot[i] + x[i] * dt
        
        # Update configuration angle
        θ_prime_tmp = q[i] + ω_prime[i] * dt
