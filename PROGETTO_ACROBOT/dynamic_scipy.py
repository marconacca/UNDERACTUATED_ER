...
def integration(q, qdot, time, torques):
    ...
    timesteps = np.linspace(0, time, 25)
    ...
    q1 = q0[0] + time*qd0[0]
    q2 = q0[1] + time*qd0[1]
    dq1 = qd0[0] + time*qdd[0]
    dq2 = qd0[1] + time*qdd[1]

    next = np.array([q1,q2,dq1,dq2])
...
def dynamics(state, tstep, torques):
    ...
    d11 = m1 * lc1**2 + m2*(l1**2 + lc2**2 + 2*l1*lc2)*np.cos(q[1]) + Iz1 + Iz2
