...
def load_trajectory_full(csv_path):
    ...
    if all((key in data.keys() for key in ["pos1", "pos2", "vel1", "vel2"])):
        traj["X"] = np.asarray([data["pos1"], data["pos2"],
                                data["vel1"], data["vel2"]]).T
    ...
    if all((key in data.keys() for key in ["pos_meas1", "pos_meas2", "vel_meas1", "vel_meas2"])):
        traj["X_meas"] = np.asarray([data["pos_meas1"], data["pos_meas2"],
                                     data["vel_meas1"], data["vel_meas2"]]).T

    if all((key in data.keys() for key in ["pos_filt1", "pos_filt2", "vel_filt1", "vel_filt2"])):
        traj["X_filt"] = np.asarray([data["pos_filt1"], data["pos_filt2"],
    ...
    if all((key in data.keys() for key in ["K11", "K12", "K13", "K14", "K21", "K22", "K23", "K24"])):
...
def concatenate_trajectories(csv_paths=[], with_tau=True):
    if type(csv_paths) == str:
        # csv_paths = [csv_paths]
        T_out, X_out, U_out = load_trajectory(csv_path='utils',
                                              with_tau=with_tau)
